...
class Log extends AbstractLogger implements LoggerInterface {
	...
	public function __construct( string $channel, array $handlers = [], array $menu_args = [] ) {
		...
		$tools_slug = 'wpify';
		if ( isset( $this->menu_args['menu_slug'] ) && 'wpify-logs' !== $this->menu_args['menu_slug'] ) {
	...
	public function debug( $message, array $data = [] ): void { $this->log( 'debug', $message, $data ); }
	public function info( $message, array $data = [] ): void { $this->log( 'info', $message, $data ); }
	public function notice( $message, array $data = [] ): void { $this->log( 'notice', $message, $data ); }
	public function warning( $message, array $data = [] ): void { $this->log( 'warning', $message, $data ); }
	public function error( $message, array $data = [] ): void { $this->log( 'error', $message, $data ); }
	public function critical( $message, array $data = [] ): void { $this->log( 'critical', $message, $data ); }
	public function alert( $message, array $data = [] ): void { $this->log( 'alert', $message, $data ); }
	public function emergency( $message, array $data = [] ): void { $this->log( 'emergency', $message, $data ); }
	...
	public function log( $level, $message, array $context = [] ): void {
		...
		if ( $this->wc_logger && $this->use_wc_logger ) {
